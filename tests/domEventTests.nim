...
proc dragDivDragStart(ev: Event) =
...
proc test12focus1(ev: Event) =
  ev.stopImmediatePropagation()

proc test12focus2(ev: Event) =
  test12Failure = true
  check false, "Unexpected listener invocation."

proc test12focus3(ev: Event) =
  test12Failure = true
  check false, "Unexpected listener invocation."
...
proc test10inputOnfocus(ev: Event) =
  if testRunning[9]:
...
proc mouseMove(ev: Event) =
  discard

proc mouseDown(ev: Event) =
...
proc dragDivDrag(ev: Event) =
  dragEventsCompleted[Drag] = true
  ev.preventDefault()

proc dragDivDragLeave(ev: Event) =
  dragEventsCompleted[DragLeave] = true
  if testRunning[0]:
    log "[dragLeave]"
  ev.preventDefault()

proc dragDivDragExit(ev: Event) =
  dragEventsCompleted[DragExit] = true
  if testRunning[0]:
    log "[dragExit]"
  ev.preventDefault()

proc dragDivDragEnd(ev: Event) =
  dragEventsCompleted[DragEnd] = true
  if testRunning[0]:
    log "[dragEnd]"
  ev.preventDefault()

proc dragDivDragEnter(ev: Event) =
