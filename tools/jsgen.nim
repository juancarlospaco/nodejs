  ...
  if result == nil:
    ...
    if s.kind != skField:
      if m.config.hcrOn:
        ...
        result.add("_")
        result.add($autoincrementingId)
        inc autoincrementingId
    s.loc.r = result
...
proc getTemp(p: PProc, defineInLocals: bool = true): Rope =
  inc(p.unique)
  result = "temporary_$1" % [rope(p.unique)]
  if defineInLocals:
...
proc genWhileStmt(p: PProc, n: PNode) =
  ...
  lineF(p, "label_$1: while (true) {$n", [labl])
  p.nested:
    gen(p, n[0], cond)
    let arrai = [cond.res, labl]
    lineF(p, "if (!$1) {$n", arrai)
    p.nested: lineF(p, "break label_$2;$n", arrai)
    lineF(p, "}$n", arrai)
    genStmt(p, n[1])
  lineF(p, "}$n", [labl])
  setLen(p.blocks, p.blocks.len - 1)
...
proc genTry(p: PProc, n: PNode, r: var TCompRes) =
  ...
  if catchBranchesExist:
    p.body.addf("--excHandler;$n",         [])
    p.body.addf("} catch (EXCEPTION) {$n", [])
    p.body.addf("  var prevJSError = lastJSError;$n", [])
    p.body.addf("  lastJSError = EXCEPTION;$n", [])
    p.body.addf("  --excHandler;$n", [])
    if hasFrameInfo(p):
...
proc genBlock(p: PProc, n: PNode, r: var TCompRes) =
  ...
  if n[0].kind != nkEmpty:
    ...
    sym.position = idx+1
  let labl = p.unique
  lineF(p, "label_$1: do {$n", [labl.rope])
...
proc genBreakStmt(p: PProc, n: PNode) =
  ...
  lineF(p, "break label_$1;$n", [rope(p.blocks[idx].id)])
